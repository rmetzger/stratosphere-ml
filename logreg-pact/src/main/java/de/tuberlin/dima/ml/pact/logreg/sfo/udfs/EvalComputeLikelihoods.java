 ...
 */
public class EvalComputeLikelihoods extends CrossFunction {
  ...
  @Override
  public void cross(Record testRecord, Record baseModelAndCoefficients,
      Collector<Record> out) throws Exception {

    int yi = testRecord.getField(IDX_INPUT1_INPUT_RECORD, IntValue.class).getValue();
    ...
    for (Vector.Element feature : xi.nonZeroes()) {
      ...
      if (! baseModel.isFeatureUsed(dim)) {
    	...
    	if (coefficient != 0) {
          ...
          recordOut.setField(IDX_OUT_DIMENSION, new IntValue(dim));
          recordOut.setField(IDX_OUT_LL_BASE, new DoubleValue(llNew - llBase));
