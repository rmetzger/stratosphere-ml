...
/**
 * 
 *<p>ItemBased Collaborative Filtering</p>
 * 
...
public class ItemSimilarityPlan implements Program, ProgramDescription{
	...
	public String getDescription() {
		return "Usage:([numSubtasks]) [inputPath] [outputPath] [sim_measure] [mostSimilar] [maxPrefPerUser] [threshold] [randomSeed]";
	}
	...
	*/
	...
	/**
	 * CooccurrencesCross computes partial co-occurrences of item pairs and emits (item,<partial co-occurrences>)
	 */					
	CrossOperator cooccurence = CrossOperator.builder(new CooccurrencesCross(sim_measure,threshold))
			.input1(itemVecReducer)
			.input2(normReducer)
			.name("Cooccurrences-Cross")
			.build();
	/**
	 * Takes the partial co-occurrences vector which is grouped by item and aggregates to form complete vector of co-occurences	
	 */
	ReduceOperator simReducer = ReduceOperator.builder(SimilarityReducer.class, LongValue.class, 0)
				.input(cooccurence)
	...
	CrossOperator normUserCross = CrossOperator.builder(NormUserCountCross.class)
				...
				.build();
	/**
	 * The output of NormUserCountCross crossed with co-occurrence vector to compute the actual similarity value using Mahout functions. 
	 */
	CrossOperator crossContract = CrossOperator.builder(new SimilarityCross(sim_measure,threshold))
				.input1(simReducer)
				.input2(normUserCross)
				.name("Similarity-Cross")
				.build();
	/**
