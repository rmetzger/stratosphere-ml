...
public class UserVectorReducer extends ReduceFunction implements Serializable {
	...
	public UserVectorReducer(String sim_measure,int maxPrefPerUser,long randomSeed,double threshold) {
	...
	@Override
	public void reduce(Iterator<Record> records,
			Collector<Record> collector) throws Exception {
		
		Vector norms = new RandomAccessSparseVector(ItemSimilarityPlan.MAX_INT_VALUE);
		Vector userVector = new RandomAccessSparseVector(ItemSimilarityPlan.MAX_INT_VALUE);
		...
		while (records.hasNext()) {
			...
			itemPVector = currentRecord.getField(1,
					ItemPrefVector.class);
		...
		//CountObservationsMapper
				Vector columnCounts = new RandomAccessSparseVector(ItemSimilarityPlan.MAX_INT_VALUE);
				
			      for (Vector.Element elem : userVector.nonZeroes()) {
			    	  int index = TasteHadoopUtils.idToIndex(elem.index());
			    	  //System.out.println("-->"+elem.index());
			        columnCounts.setQuick(index, columnCounts.getQuick(elem.index()) + 1);
			      }
		 //sampling down
	...
	//	System.out.println(pactLong.getValue()+" ---->"+rowVector);
	 ...
	 private Vector sampleDown(Vector rowVector,Vector columnCounts) {
		 ...
		 //long randomSeed = this.seed;
	       ...
	       double rowSampleRate = (double) Math.min(this.maxPrefPerUser, observationsPerRow) / (double) observationsPerRow;
	      ...
	      // long usedObservations = 0;
	       ...
	       for (Vector.Element elem : rowVector.nonZeroes()) {
	         ...
	         double columnSampleRate = (double) Math.min(this.maxPrefPerUser, columnCount) / (double) columnCount;
